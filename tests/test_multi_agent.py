...
from ai_trading_agents.multi_agent import (
    AgentVote, trend_agent_h4, momentum_agent_h1, timing_agent_m30, vote_all,
)
...
def test_vote_all_requires_unanimity():
    ...
    try:
        d, _ = ma.vote_all({"H4": pd.DataFrame(), "H1": pd.DataFrame(),
                            "M30": pd.DataFrame()}, min_votes=3)
        assert d == 0
        d2, _ = ma.vote_all({"H4": pd.DataFrame(), "H1": pd.DataFrame(),
                             "M30": pd.DataFrame()}, min_votes=2)
        assert d2 == +1
    finally:
