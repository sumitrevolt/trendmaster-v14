...
def test_balanced_mix_scales_above_one():
    ...
    cfg = KellyConfig(min_samples=4, kelly_fraction=0.5, floor_fraction=0.25, max_fraction=2.0)
...
def test_shadow_mode_does_not_apply():
    cfg = KellyConfig(min_samples=3, kelly_fraction=0.5, shadow_mode=True)
...
def test_floor_caps_downside():
    cfg = KellyConfig(min_samples=4, kelly_fraction=0.5, floor_fraction=0.5, max_fraction=2.0)
