...
from ai_trading_agents.risk_manager import (
    Position, RiskConfig, RiskState, check_risk, size_position, team_of,
)
...
def test_size_position_scales_with_equity():
    ...
    small = size_position(equity=1_000,  risk_pct=1.0,
                          sl_distance_price=10, pip_value_per_lot=1.0, cfg=cfg)
    big   = size_position(equity=10_000, risk_pct=1.0,
                          sl_distance_price=10, pip_value_per_lot=1.0, cfg=cfg)
    assert big > small
...
def test_size_position_caps_at_max_lot():
    ...
    lots = size_position(equity=1_000_000, risk_pct=5.0,
                         sl_distance_price=1, pip_value_per_lot=1.0, cfg=cfg)
...
def test_global_concurrency_blocks_trade():
    state = RiskState(equity=1000, start_of_day=1000,
                      open_positions=[Position(f"EURUSD", "BUY", 0.1,
                                               1.0, 0.9, 1.2)] * 6)
...
def test_team_cap_blocks():
    opens = [Position("EURUSD", "BUY", 0.1, 1, 0.9, 1.1),
...
def test_correlation_cap_blocks_stacked_longs():
    opens = [Position("EURUSD", "BUY", 0.1, 1, 0.9, 1.1),
             Position("GBPUSD", "BUY", 0.1, 1, 0.9, 1.1)]
    state = RiskState(equity=1000, start_of_day=1000, open_positions=opens)
    cfg = RiskConfig(max_corr_same_dir=2, max_open_total=10,
                     max_open_per_team=10)
...
def test_loss_streak_blocks_after_n_losses():
    ...
    state = RiskState(equity=990, start_of_day=1000, open_positions=[],
