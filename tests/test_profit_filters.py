...
from ai_trading_agents.profit_filters import (
    spread_guard, volatility_regime, daily_profit_lock,
    loss_streak_cooldown, session_window, evaluate_all,
)
...
def test_evaluate_all_happy_path():
    s = _atr_sample()
    d = evaluate_all(
        spread_price=0.05, atr_price=1.0,
        atr_series=s,
        equity_now=1010, equity_start_of_day=1000,
...
def test_evaluate_all_any_veto_blocks():
    s = _atr_sample()
    d = evaluate_all(
        spread_price=0.50, atr_price=1.0,        # spread too wide → VETO
        atr_series=s,
        equity_now=1010, equity_start_of_day=1000,
        recent_results=[5, -2, 3],
        now_utc=datetime(2026, 4, 22, 13, 0, tzinfo=timezone.utc),
