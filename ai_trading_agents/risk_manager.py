...
def size_position(
    equity: float, risk_pct: float, sl_distance_price: float, pip_value_per_lot: float, cfg: Optional[RiskConfig] = None
) -> float:
...
def _corr_conflict(symbol: str, direction: str, positions: Iterable[Position], max_same_dir: int) -> Optional[str]:
    for group in _CORR_GROUPS:
        if symbol not in group:
            continue
        same_dir = [p for p in positions if p.symbol in group and p.direction == direction]
        if len(same_dir) >= max_same_dir:
            return f"correlation cap: {len(same_dir)} {direction} in group {sorted(group)}"
    return None
...
def check_risk(
    symbol: str, direction: str, lots: float, state: RiskState, cfg: Optional[RiskConfig] = None
) -> RiskDecision:
    cfg = cfg or RiskConfig()
    ...
    if state.start_of_day > 0:
        ...
        if dd_pct >= cfg.max_daily_loss_pct:
            return RiskDecision(False, f"daily loss stop: {dd_pct:.2f}% >= {cfg.max_daily_loss_pct:.2f}%")

        gain_pct = (state.equity - state.start_of_day) / state.start_of_day * 100.0
        if gain_pct >= cfg.daily_profit_target_pct:
            return RiskDecision(
                False, f"daily profit lock: +{gain_pct:.2f}% >= target {cfg.daily_profit_target_pct:.2f}%"
            )
    ...
    if streak >= cfg.max_consec_losses:
        return RiskDecision(False, f"loss streak {streak} >= cap {cfg.max_consec_losses}")

    if len(state.open_positions) >= cfg.max_open_total:
        return RiskDecision(False, f"max open total {cfg.max_open_total} reached")

    t = team_of(symbol)
    team_open = [p for p in state.open_positions if team_of(p.symbol) == t]
    if len(team_open) >= cfg.max_open_per_team:
        return RiskDecision(False, f"team {t} has {len(team_open)} open >= cap {cfg.max_open_per_team}")

    corr_reason = _corr_conflict(symbol, direction, state.open_positions, cfg.max_corr_same_dir)
    if corr_reason:
