...
def _adx_wilder(high: pd.Series, low: pd.Series, close: pd.Series,
                n: int) -> pd.Series:
...
def _supertrend(high: pd.Series, low: pd.Series, close: pd.Series,
    ...
    if n <= start:
        return (pd.Series(st_line, index=close.index),
                pd.Series(st_dir,  index=close.index))
    ...
    return (pd.Series(st_line, index=close.index),
            pd.Series(st_dir,  index=close.index, dtype=int))


# ─── master function ──────────────────────────────────────────────────────
def compute_confirmations(df: pd.DataFrame,
                          params: Optional[EAParams] = None) -> pd.DataFrame:
    """
    ...
    out["agreed"] = (out["c1_trend"].astype(int)
                     + out["c2_vola"].astype(int)
                     + out["c3_momo"].astype(int))

    return out


def ea_would_enter(df: pd.DataFrame,
                   params: Optional[EAParams] = None,
                   shift: int = 1) -> int:
    """
