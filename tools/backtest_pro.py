...
from tools.backtest_filtered import (
    _adx, _atr, _bollinger, _ema, _macd_hist, _super_trend,
)
...
def backtest_pro(df: pd.DataFrame, cfg: ProConfig, name: str = "pro") -> ProResult:
    ...
    macd_v = macd_h.values; st_v = st.values
    ...
    while i < n - cfg.hold_bars:
        ...
        if (st_v[i] > 0
                ...
                and macd_v[i] > macd_v[i-1] and macd_v[i] > 0):
            direction = 1
        elif (st_v[i] < 0
                and ema20_v[i] < ema50_v[i] < ema200_v[i]
                and closes[i] < ema50_v[i]
        ...
        for j in range(i + 1, j_end):
            ...
            if not partial_done:
                touched_partial = ((hi >= partial_tp) if direction > 0
                                    else (lo <= partial_tp))
                if touched_partial:
                    ...
                    if cfg.trail_after_partial:
                        current_sl = max(current_sl, entry) if direction > 0 \
                            else min(current_sl, entry)

            # Did full TP hit (remaining portion)?
            touched_full = ((hi >= full_tp) if direction > 0
                              else (lo <= full_tp))
            if touched_full:
            ...
            # Did SL get hit?
            touched_sl = ((lo <= current_sl) if direction > 0
                            else (hi >= current_sl))
            if touched_sl:
                # How far from entry?
    ...
    pos_timeouts = sum(1 for r in r_outcomes[-result.timeouts:]
                       if result.timeouts and r > 0)
    eff_w += pos_timeouts
...
def main():
    ...
    configs = [
        ("basic_BE", ProConfig(adx_min=25, break_even_at_r=1.0,
                                partial_tp_at_r=1.5, trail_after_partial=True)),
        ("BE_adx30", ProConfig(adx_min=30, break_even_at_r=1.0,
                                partial_tp_at_r=1.5, trail_after_partial=True)),
        ("BE_adx30_rising", ProConfig(adx_min=30, break_even_at_r=1.0,
                                       partial_tp_at_r=1.5, trail_after_partial=True,
                                       require_adx_rising=True,
                                       min_extra_confluences=1)),
        ("BE_adx30_h1aligned", ProConfig(adx_min=30, break_even_at_r=1.0,
                                          partial_tp_at_r=1.5, trail_after_partial=True,
                                          require_h1_aligned=True,
                                          min_extra_confluences=1)),
        ("BE_adx30_rising_h1", ProConfig(adx_min=30, break_even_at_r=1.0,
                                          partial_tp_at_r=1.5, trail_after_partial=True,
                                          require_adx_rising=True,
                                          require_h1_aligned=True,
                                          min_extra_confluences=2)),
        ("BE_adx35_rising_h1_vol", ProConfig(adx_min=35, break_even_at_r=1.0,
                                               partial_tp_at_r=1.5,
        ...
        ("tight_BE05_adx35_all", ProConfig(adx_min=35, break_even_at_r=0.5,
                                             partial_tp_at_r=1.0,
                                             trail_after_partial=True,
                                             require_adx_rising=True,
                                             require_h1_aligned=True,
    ...
    print()
    hdr = (f"{'config':32s} {'n':>5s} {'rawWR':>6s} {'effWR':>6s} "
    ...
    for name, cfg in configs:
        ...
        if r.effective_wr >= 0.80 and r.trades >= 20:
            mark = " << 80% eff WR"
        print(f"{r.name:32s} {r.trades:5d} {r.raw_wr*100:5.1f}% "
