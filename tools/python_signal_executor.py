...
# ScalpEngine trade-management config lives in config/settings.py (SCALPING).
try:
...
MAGIC_QUICK = 14014
MAGIC_TREND = 14015
MAGIC_SCALP = 241000  # ScalpEngine pending-limit legs (separate from QUICK/TREND)
...
_OUR_MAGICS = (MAGIC_QUICK, MAGIC_TREND, MAGIC, MAGIC_SCALP, 0)
...
def has_pending_in_dir(symbol: str, want_buy: bool) -> bool:
    """True if WE already have a pending (limit/stop) order in this direction.
    Prevents stacking multiple scalp limit orders before the first fills."""
    try:
...
def load_signal(path: Path) -> Optional[dict]:
    ...
    ALLOWED_STRATEGIES = {
        ...
        # [2026-08-27] ScalpEngine pending-limit signals
        "scalp",
        # [2026-08-25] Brain-generated signals (TrendMaster rule/ml inference)
        # write signal files without tv_strategy; allow empty so the
        # executor processes them instead of silently dropping all 7 files.
        "",
    }
...
def _send_one_leg(symbol: str, direction: str, leg_name: str, magic: int,
    ...
    is_scalp = sig.get("tv_strategy") == "scalp"
    if is_scalp:
        sl_atr_mult = float(sig.get("sl_atr_mult", sl_atr_mult))
        tp_atr_mult = float(sig.get("tp_atr_mult", tp_atr_mult))
    ...
    is_limit = is_scalp and order_kind == "LIMIT" and entry_price not in (None, 0)
    ...
    tf = sig.get("mode") or sig.get("tv_timeframe", "?") or "?"
    ...
    res = mt5.order_send(request)
    # Retry with alternate filling on 10030 (market orders only)
    if res and res.retcode == 10030 and action == mt5.TRADE_ACTION_DEAL:
    ...
    kind = "LIMIT" if is_limit else "MARKET"
    msg = (
        f"<b>{leg_emoji} {emoji} {symbol} {direction} {leg_name} ({kind})</b>\n"
        f"Entry: <code>{price:.5f}</code>  Lots: <b>{lots}</b>\n"
        f"SL: <code>{sl:.5f}</code>  TP: <code>{tp:.5f}</code>\n"
        f"RR: <b>1:{rr:.1f}</b>  ATR: {atr:.5f}\n"
        f"Source: ScalpEngine ({tf})  Conf: {conf:.0f}%"
    )
    if leg_name == "TREND":
        msg += f"\n<i>Trailing SL activates at +{TRAIL_ACTIVATE_ATR}xATR favorable</i>"
    _tg_send(msg)
    return True
...
def place_order(symbol: str, sig: dict) -> bool:
    ...
    # [2026-08-27] SCALPING: single pending LIMIT leg. No flip logic, no
    # two-leg split. The engine sets SL/TP; safeguards above already gate it.
    if sig.get("tv_strategy") == "scalp":
        if has_pending_in_dir(symbol, direction == "BUY"):
            log.info("skip %s %s: scalp pending already open in dir", symbol, direction)
            return False
        info = mt5.symbol_info(symbol)
        if not info:
            return False
        ...
        if atr is None or atr <= 0:
            log.warning("no ATR for scalp %s - skip", symbol)
            return False
        ok = _send_one_leg(symbol, direction, "SCALP", MAGIC_SCALP, QUICK_SL_ATR, QUICK_TP_ATR, atr, info, sig)
        if ok:
            _LAST_ORDER[(symbol, direction)] = time.time()
            _save_cooldown()
        return ok

    # [2026-05-17 operator policy] Opposite-direction flip requires EXPLICIT
    # brain agreement. brain_agrees_with_signal in safeguards is permissive
    # (defers to RP when brain has no view). For flips we need stricter —
    # brain must explicitly endorse the new direction. If brain has no view,
    # is stale, weak conf, or disagrees → KEEP existing position, skip new signal.
    opposite_positions = [
        p for p in positions_for_symbol(symbol)
        if _is_our_position(p) and ((p.type == mt5.ORDER_TYPE_BUY) != (direction == "BUY"))
    ]
    if opposite_positions:
        try:
            from tools.safeguards import brain_explicitly_agrees as _bea
        except ImportError:
            try:
                from safeguards import brain_explicitly_agrees as _bea
            except ImportError:
                _bea = None
        if _bea is not None:
            agreed, reason = _bea(symbol, direction)
            if not agreed:
                log.info(
                    "FLIP BLOCKED %s %s: %s (keeping %d opposite position(s))",
                    symbol, direction, reason, len(opposite_positions),
                )
                _tg_send(
                    f"<b>🚫 {symbol} FLIP BLOCKED</b>\n"
                    f"RP says: {direction}\n"
                    f"Currently holding: {len(opposite_positions)} opposite position(s)\n"
                    f"Brain check: {reason}"
                )
                return False
            log.info(
                "FLIP APPROVED %s %s: %s — closing %d opposite position(s)",
                symbol, direction, reason, len(opposite_positions),
            )
        else:
            log.warning(
                "FLIP: brain_explicitly_agrees not importable — "
                "proceeding with legacy flip behavior"
            )
...
def _print_startup_banner() -> None:
    ...
    # Keep this list in sync with load_signal's ALLOWED_STRATEGIES (line ~371).
    # 2026-05-13: added rocket_prime_chart_ocr + telegram_manual_direction;
    # removed local_generator (Rocket-Prime-only mode per operator).
    expected_strategies = {
        "rocket_prime", "rocket_prime_text", "rocket_prime_inferred",
        "rocket_prime_plot0", "rocket_prime_plot1", "rocket_prime_url_direction",
        "rocket_prime_chart_ocr",       # async OCR direction (Priority 1.5)
        "telegram_manual_direction",    # operator-tapped BUY/SELL via Telegram
    }
    log.info("[OK] ALLOWED_STRATEGIES = %s", sorted(expected_strategies))
    log.info("=" * 60)


def _process_one(symbol: str, path: Path) -> Tuple[int, int, int]:
    """Load + freshness-check + execute one signal file. Returns
    (placed, skipped_stale, skipped_filter)."""
    placed = 0
    skipped_stale = 0
    skipped_filter = 0
    ...
    if sig is None:
        skipped_filter += 1
        return placed, skipped_stale, skipped_filter
    if not signal_is_fresh(sig):
        age = int(time.time() - (sig.get("ts") or 0))
        log.info("skip %s: stale signal (age=%ds > limit=%ds)", symbol, age, SIGNAL_MAX_AGE_S)
        skipped_stale += 1
        try:
            archive_dir = path.parent / "stale_archive"
            archive_dir.mkdir(exist_ok=True)
            from datetime import datetime as _dt
            stamp = _dt.fromtimestamp(sig.get("ts") or time.time()).strftime("%Y%m%d_%H%M%S")
            dst = archive_dir / f"{path.stem}_{stamp}{path.suffix}"
            path.replace(dst)
            log.info("  -> archived stale %s -> %s (age=%ds)", path.name, dst.name, age)
        except Exception as _e:
            log.warning("  -> archive failed for stale %s: %s", path.name, _e)
        return placed, skipped_stale, skipped_filter
...
# ── ScalpEngine position management (mirrors validated backtest exit) ───────
# In-memory state for the live process; lost on restart (acceptable — the
# pending-limit order's own SL/TP is the safety net if state is lost).
_SCALP_STATE: Dict[int, dict] = {}


def _mt5_scalp_close(ticket: int, symbol: str, volume: float, close_type: int) -> bool:
    """Close `volume` of scalp position `ticket`. close_type = ORDER_TYPE_SELL
    to close a BUY, ORDER_TYPE_BUY to close a SELL."""
    req = {
        "action": mt5.TRADE_ACTION_DEAL,
        "symbol": symbol,
        "volume": volume,
        ...
        "deviation": DEVIATION_PTS,
        "magic": MAGIC_SCALP,
        "comment": f"{COMMENT_PREFIX}_scale",
    }
    res = mt5.order_send(req)
    if res is None or res.retcode != mt5.TRADE_RETCODE_DONE:
        log.warning("scalp close FAIL ticket=%d vol=%.4f rc=%s",
                    ticket, volume, getattr(res, "retcode", "none"))
        return False
    log.info("scalp close OK ticket=%d vol=%.4f", ticket, volume)
    return True


def _mt5_scalp_be(ticket: int, symbol: str, new_sl: float, tp: float) -> bool:
    req = {
    ...
    res = mt5.order_send(req)
    if res is None or res.retcode != mt5.TRADE_RETCODE_DONE:
        log.warning("scalp BE-modify FAIL ticket=%d rc=%s", ticket, getattr(res, "retcode", "none"))
        return False
    log.info("scalp BE-modify OK ticket=%d sl=%.5f", ticket, new_sl)
    return True


def _manage_scalp_positions() -> None:
    """Manage open ScalpEngine positions to match tools/backtest_scalping.py:
    close `scale_out.frac` at +1R, move remainder SL to break-even, and force
    close at `max_hold_bars` (time stop). Mean-reversion exits full (scale_out
    only applies to SMC; but MR never emits tp1 so it is skipped here anyway)."""
    if settings is None:
        return
    sc = getattr(settings, "SCALPING", {})
    so = sc.get("scale_out", {})
    if not so.get("enabled", False):
        return
    frac = float(so.get("frac", 0.5))
    max_hold = int(sc.get("max_hold_bars", 0))
    max_hold_s = max_hold * 300  # M5 bars are 5 min each
    now = time.time()
    positions = mt5.positions_get(magic=MAGIC_SCALP)
    if not positions:
        _SCALP_STATE.clear()
        return
    live: set = set()
    for pos in positions:
        live.add(pos.ticket)
        st = _SCALP_STATE.get(pos.ticket)
        if st is None:
            st = {"scaled": False}
            _SCALP_STATE[pos.ticket] = st
        if not pos.sl or pos.sl == 0.0:
            continue  # no SL -> cannot compute 1R; rely on fixed SL/TP
        ...
        if pos.type == mt5.ORDER_TYPE_BUY:
            at_r1 = tick.bid >= entry + sl_dist
            close_type = mt5.ORDER_TYPE_SELL
        else:
            at_r1 = tick.ask <= entry - sl_dist
            close_type = mt5.ORDER_TYPE_BUY
        if not st["scaled"] and at_r1:
            vol = pos.volume * frac
            sinfo = mt5.symbol_info(pos.symbol)
            vmin = getattr(sinfo, "volume_min", 0.0) or 0.0
            if vol >= vmin:
                if _mt5_scalp_close(pos.ticket, pos.symbol, vol, close_type):
                    st["scaled"] = True
                    _mt5_scalp_be(pos.ticket, pos.symbol, entry, tp)
        if max_hold_s > 0 and (now - pos.time) >= max_hold_s:
            rem = pos.volume * (1 - frac) if st["scaled"] else pos.volume
            _mt5_scalp_close(pos.ticket, pos.symbol, rem, close_type)
            _SCALP_STATE.pop(pos.ticket, None)
    for tk in list(_SCALP_STATE.keys()):
        if tk not in live:
            _SCALP_STATE.pop(tk, None)
        ...
        try:
            ...
            for symbol in SYMBOLS:
                ...
                if not path.exists():
                    ...
                    p, st, sf = _process_one(symbol, path)
                    placed += p
                    skipped_stale += st
                    skipped_strategy_filter += sf
                # ScalpEngine signals (separate files, pending limit orders)
                scalp_path = sig_dir / f"trendmaster_scalp_{symbol}.json"
                if scalp_path.exists():
                    p2, st2, sf2 = _process_one(symbol, scalp_path)
                    placed += p2
                    skipped_stale += st2
                    skipped_strategy_filter += sf2
                else:
                    skipped_no_file += 1
            # Manage open ScalpEngine positions (1R partial + BE + max-hold).
            _manage_scalp_positions()
