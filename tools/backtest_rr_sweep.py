...
def main():
    ...
    print()
    print(f"{'config':30s} {'SL':>4s} {'TP':>4s} {'RR':>6s} "
          f"{'trades':>6s} {'WR':>7s} {'exp(R)':>8s} {'gross':>9s}")
    print("-" * 90)
    ...
    for sl, tp in rr_targets:
        ...
        for adx in adx_mins:
            ...
            if r.win_rate >= 0.80 and r.trades >= 30:
                mark = " [PASS!]"
            print(f"sl{sl}tp{tp}_adx{adx:<2d}{'':14s} "
            ...
            if key not in best_by_rr or r.win_rate > best_by_rr[key][1]:
                best_by_rr[key] = (f"adx{adx} sl{sl}/tp{tp}", r.win_rate,
                                    r.expectancy, r.trades, r.gross_r)
    ...
    for rr, (name, wr, exp, n, gross) in sorted(best_by_rr.items()):
        ...
        print(f"  {rr:6s}  {name:30s}  WR={wr*100:5.1f}%  exp={exp:+6.3f}R  "
              f"n={n:4d}  gross={gross:+8.2f}R{flag}")
