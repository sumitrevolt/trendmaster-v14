...
def _detect_bos(highs: np.ndarray, lows: np.ndarray, i: int,
                 lookback: int = 20) -> int:
...
def backtest_v15(df: pd.DataFrame, df_h1: pd.DataFrame, df_h4: pd.DataFrame,
                  sl_atr: float = 1.0, tp_atr: float = 2.0,
                  partial_r: float = 1.0, be_r: float = 1.0,
                  min_filters: int = 6,
                  hold_bars: int = 48, cooldown: int = 20) -> Dict:
    ...
    while i < n - hold_bars:
        ...
        for j in range(i + 1, j_end):
            ...
            if touched_ft:
                ...
                if partial_done:
                    outcome_kind = "partial_full"; partial_full += 1
                else:
                    outcome_kind = "full_win"; full_wins += 1
                break
            # SL
            ...
            if touched_sl:
                ...
                if partial_done and be_active:
                    outcome_kind = "partial_win"; partial_wins += 1
                elif be_active:
                    outcome_kind = "be"; be_stops += 1
                else:
                    outcome_kind = "loss"; losses += 1
                break
        ...
        if outcome_kind is None:
            ...
            outcome_kind = "timeout"; timeouts += 1
    ...
    if trades == 0:
        return {"trades": 0, "raw_wr": 0.0, "effective_wr": 0.0,
                "expectancy": 0.0, "gross_r": 0.0, "sharpe": 0.0,
                "full_wins": 0, "partial_full": 0, "partial_wins": 0,
                "be_stops": 0, "losses": 0, "timeouts": 0}
...
def main():
    ...
    TEAMS = {
        ...
        "FOREX":       ["GBPJPY", "USDCAD", "USDCHF", "AUDUSD", "USDJPY",
                         "NZDUSD", "EURJPY", "AUDJPY", "CADJPY"],  # drop EURUSD/GBPUSD/EURGBP (losers)
    ...
    results = {}
    print(f"{'TEAM':12s} {'CONFIG':14s} {'trades':>6s} {'rawWR':>6s} "
    ...
    for team, symbols in TEAMS.items():
        best_eff = None
        for cfg in configs:
            pooled = {"trades": 0, "full_wins": 0, "partial_full": 0,
                       "partial_wins": 0, "be_stops": 0, "losses": 0,
                       "timeouts": 0, "gross_r": 0.0, "exps": []}
            for sym in symbols:
                ...
                r = backtest_v15(df, df_h1, df_h4,
                                  sl_atr=cfg["sl"], tp_atr=cfg["tp"],
                                  partial_r=cfg["partial_r"], be_r=cfg["be_r"],
            ...
            if pooled["trades"] == 0:
                continue
            eff_w = (pooled["full_wins"] + pooled["partial_full"]
                     + pooled["partial_wins"] + pooled["be_stops"])
            ...
            if eff_wr >= 0.67:
                ...
                mark = "[close]"
            print(f"{team:12s} {cfg['label']:14s} {pooled['trades']:>6d} "
    ...
    for team, r in results.items():
        if r is None:
            print(f"  {team:12s}  no results"); continue
        mark = "[>= 67%]" if r["eff_wr"] >= 0.67 else ""
        print(f"  {team:12s}  cfg={r['config']:14s}  effWR={r['eff_wr']*100:5.1f}%  "
