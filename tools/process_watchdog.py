...
def is_running(needle: str) -> bool:
    ...
    try:
        ...
        # [2026-08-25] Normalize separators: respawned cmdlines carry
        # backslash paths (D:\...\tools\executor.py) while EXPECTED uses
        # forward slashes ("tools/python_signal_executor.py"). The raw
        # substring match never matched, so the watchdog declared every
        # file-path process DEAD every tick and spawned endless duplicates.
        def _norm(s: str) -> str:
            return s.lower().replace("\\", "/")

        needle_lower = _norm(needle)
        for p in psutil.process_iter(["cmdline"]):
            try:
                cl = p.info.get("cmdline") or []
                if needle_lower in _norm(" ".join(cl)):
                    return True
            except Exception:
                continue
        return False
