"""Kill dashboard → swap H1→M1 → restart dashboard."""
...
def main():
    ...
    print("Swap H1→M1…", flush=True)
    print("=" * 70, flush=True)
    p = subprocess.Popen([str(PY), str(HERE / "swap_h1_to_m1.py")],
                         cwd=str(ROOT),
    ...
    subprocess.Popen([str(PY), str(ROOT / "tools/master_autostart.py")],
                     cwd=str(ROOT),
                     stdout=open(ROOT / "logs/master_after_swap.out", "ab"),
