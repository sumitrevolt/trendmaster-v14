"""Kill dashboard → add M1 alerts → restart dashboard."""
...
def main():
    ...
    print("Add M1 alerts…", flush=True)
    print("=" * 70, flush=True)
    p = subprocess.Popen([str(PY), str(HERE / "add_m1_alerts.py")],
                         cwd=str(ROOT),
    ...
    subprocess.Popen([str(PY), str(ROOT / "tools/master_autostart.py")],
                     cwd=str(ROOT),
                     stdout=open(ROOT / "logs/master_after_m1.out", "ab"),
