"""Kill dashboard → run modify endpoint probe → restart dashboard."""
...
def main():
    ...
    print("Probe modify endpoint…", flush=True)
    print("=" * 70, flush=True)
    p = subprocess.Popen([str(PY), str(HERE / "probe_modify_endpoint.py")],
                         cwd=str(ROOT),
    ...
    subprocess.Popen([str(PY), str(ROOT / "tools/master_autostart.py")],
                     cwd=str(ROOT),
                     stdout=open(ROOT / "logs/master_after_modify.out", "ab"),
