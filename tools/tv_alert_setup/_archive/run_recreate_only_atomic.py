"""Kill dashboard → run recreate_top5_instant.py only → restart dashboard."""
...
def main():
    ...
    print("Recreate…", flush=True)
    print("=" * 70, flush=True)
    p = subprocess.Popen(
        [str(PY), str(HERE / "recreate_top5_instant.py")],
        cwd=str(ROOT),
    ...
    subprocess.Popen([str(PY), str(ROOT / "tools/master_autostart.py")],
                     cwd=str(ROOT),
                     stdout=open(ROOT / "logs/master_after_recreate.out", "ab"),
