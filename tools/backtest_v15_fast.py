...
def backtest(df, df_h1, df_h4, sl_atr=1.0, tp_atr=1.5,
              partial_r=0.5, be_r=0.5, min_filters=5,
              hold_bars=48, cooldown=15, tight_session=True):
    atr = _atr(df).values
    adx = _adx(df).values
    ...
    while i < n - hold_bars:
        ...
        # F1 BOS
        if i >= 20:
            ms_hi = np.max(highs[i-20:i]); ms_lo = np.min(lows[i-20:i])
            if closes[i] > ms_hi: direction = 1; filters_ok += 1
            elif closes[i] < ms_lo: direction = -1; filters_ok += 1
        if direction == 0:
            i += 1; continue
        # F2 FVG
        if i >= 2:
            if direction == 1 and lows[i] > highs[i-2]: filters_ok += 1
            elif direction == -1 and highs[i] < lows[i-2]: filters_ok += 1
        # F3 EMA fan M5
        ...
        if np.isfinite(vol_ma[i]) and vols[i] > 1.3 * vol_ma[i]:
            filters_ok += 1
        # F7 MACD hist rising
        if direction == 1 and macd_h[i] > macd_h[i-1] and macd_h[i] > 0: filters_ok += 1
        elif direction == -1 and macd_h[i] < macd_h[i-1] and macd_h[i] < 0: filters_ok += 1
        # F8 ADX strong
    ...
    trades = len(outcomes)
    if trades == 0:
        return {"trades": 0, "raw_wr": 0.0, "effective_wr": 0.0,
                "expectancy": 0.0, "gross": 0.0, "full": 0, "partial_full": 0,
                "partial_win": 0, "be_stops": 0, "losses": 0, "timeouts": 0}
    ...
    return {
        ...
        "gross": float(arr.sum()),
        "full": full_wins, "partial_full": partial_full,
        "partial_win": partial_wins, "be_stops": be_stops,
...
def main():
    ...
    results = {}
    print(f"{'TEAM':12s} {'CONFIG':26s} {'n':>5s} {'rawWR':>6s} "
          f"{'effWR':>6s} {'exp':>7s} {'gross':>8s}  mark")
    print("-" * 90)
    for team, syms in TEAMS.items():
        team_best = None
        t0 = time.time()
        for label, sl, tp, pr, br, mf, ts in configs:
            tot = {"trades":0,"full":0,"partial_full":0,"partial_win":0,
                    "be_stops":0,"losses":0,"timeouts":0,"gross":0.0}
            for sym in syms:
                ...
                r = backtest(df, df_h1, df_h4, sl_atr=sl, tp_atr=tp,
                              partial_r=pr, be_r=br, min_filters=mf,
                              tight_session=ts)
                for k in ("trades","full","partial_full","partial_win","be_stops","losses","timeouts"):
                    tot[k] += r.get(k, 0)
                tot["gross"] += r.get("gross", 0.0)
            if tot["trades"] == 0: continue
            ...
            mark = ""
            if eff_wr >= 0.67: mark = "[>= 67%]"
            elif eff_wr >= 0.60: mark = "[close]"
            print(f"{team:12s} {label:26s} {tot['trades']:>5d} "
                  f"{raw_wr*100:5.1f}% {eff_wr*100:5.1f}% "
                  f"{exp:+7.3f} {tot['gross']:+8.2f}  {mark}")
            if team_best is None or eff_wr > team_best["eff_wr"]:
                team_best = {"config": label, "sl": sl, "tp": tp,
                              "partial_r": pr, "be_r": br, "min_filt": mf,
                              "tight_session": ts,
                              "trades": tot["trades"],
                              "eff_wr": eff_wr, "raw_wr": raw_wr,
                              "exp": exp, "gross": tot["gross"]}
            sys.stdout.flush()
        dt = time.time() - t0
    ...
    for team, r in results.items():
        ...
        print(f"  {team:12s}  cfg={r['config']:25s}  effWR={r['eff_wr']*100:5.1f}%  "
