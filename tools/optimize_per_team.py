...
TEAMS = {
    ...
    "FOREX":       ["GBPJPY", "USDCAD", "USDCHF", "EURUSD", "GBPUSD",
...
def team_backtest(team: str, symbols: list, data_dir: Path,
                   sl: float, tp: float, adx: int) -> dict:
    """Run a config across every symbol in the team, pool the results."""
    totals = {"trades": 0, "wins": 0, "losses": 0, "timeouts": 0,
              "gross_r": 0.0, "per_symbol": {}}
    all_outcomes = []
    for sym in symbols:
    ...
    for sl in grid["sl"]:
        for tp in grid["tp"]:
            for adx in grid["adx"]:
                ...
                rr = tp / sl
                row = {
                    "sl": sl, "tp": tp, "adx": adx, "rr": round(rr, 2),
                    "trades":     r["trades"],
                    "win_rate":   r["win_rate"],
...
def main():
    ...
    summary = {}
    print(f"{'TEAM':12s}  {'SL':>5s} {'TP':>5s} {'RR':>5s} {'ADX':>4s}  "
    ...
    for team, syms in TEAMS.items():
        ...
        b = out.get("best")
        if not b:
            print(f"{team:12s}  (no data)"); continue
        print(f"{team:12s}  {b['sl']:>5.1f} {b['tp']:>5.1f} 1:{b['rr']:<3.1f} "
              f"{b['adx']:>4d}  {b['trades']:>6d} {b['win_rate']*100:5.1f}% "
              f"{b['expectancy']:+8.3f} {b['gross_r']:+9.2f} {dt:>5.1f}s")
        sys.stdout.flush()

    # Write JSON
    (reports / "PER_TEAM_OPTIMAL.json").write_text(
    ...
    md = ["# Per-Team Optimal Parameters",
          "",
    ...
    for team, data in summary.items():
        b = data.get("best")
        if not b:
            md.append(f"| {team} | — | — | — | — | — | — | — | — | — |"); continue
        md.append(f"| {team} | {b['sl']} | {b['tp']} | 1:{b['rr']} | {b['adx']} | "
    ...
    for team, data in summary.items():
        ...
        md.append(f"### {team}")
        md.append("")
        md.append(f"Winning config: **SL={b['sl']}×ATR  TP={b['tp']}×ATR  "
                  f"1:{b['rr']} R:R  ADX≥{b['adx']}**")
        md.append("")
        ...
        for sym, r in (b.get("per_symbol") or {}).items():
            md.append(f"| {sym} | {r['trades']} | {r['win_rate']*100:.1f}% | "
                      f"{r['gross_r']:+.2f} |")
