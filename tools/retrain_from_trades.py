...
def retrain(symbol: str,
            ...
            history_csv: Optional[str] = None) -> int:
    ...
    df = pd.merge_asof(labels, feats_reset.sort_values("time"),
    ...
    tr = df.iloc[:split]
    va = df.iloc[split:]
    logger.info("train=%d  val=%d  base-rate=%.2f%%",
                len(tr), len(va), df["won_R"].mean() * 100)
    ...
    if take.any():
        ...
        logger.info("holdout precision @ p≥0.60: %.2f%%  (n=%d)",
                    prec * 100, int(take.sum()))
    else:
...
def main() -> int:
    ...
    return retrain(p.parse_args().symbol,
                   p.parse_args().trades,
                   p.parse_args().history)
