...
def backtest(df, sl_mul, tp_mul, adx_min,
             hold=36, cooldown=20, warmup=210):
    ...
    while i < n - hold:
        ...
        if (ema20[i] > ema50[i] > ema200[i]
                ...
                and macd_h[i] > macd_h[i-1] and macd_h[i] > 0):
            direction = 1
        ...
        for j in range(i+1, j_end):
            ...
            if hit_sl and hit_tp:
                outcome = -1.0; losses += 1; break
            if hit_sl:
                outcome = -1.0; losses += 1; break
            if hit_tp:
                outcome = tp_mul / sl_mul; wins += 1; break
        if outcome is None:
            last_close = closes[j_end-1]
            mtm = (last_close - entry) / sl_dist
            if direction < 0: mtm = -mtm
            outcome = mtm; timeouts += 1
            if outcome > 0: wins += 1
            else: losses += 1
    ...
    if trades == 0:
        return {"trades": 0, "wins": 0, "losses": 0, "timeouts": 0,
                "win_rate": 0.0, "expectancy": 0.0, "gross_r": 0.0, "sharpe": 0.0}
        ...
        for tp in TP_VALUES:
            for adx in ADX_VALUES:
                ...
                if r["trades"] == 0:
                    continue
                rr = tp / sl
                row = {"sl": sl, "tp": tp, "rr": round(rr, 2),
                       "adx": adx, **r, "score": round(score(r), 3)}
...
def main():
    ...
    summary = {}
    print(f"Optimizing {len(SYMBOLS)} symbols x "
    ...
    for idx, sym in enumerate(SYMBOLS, 1):
        ...
        dt = time.time() - t0
        if out is None:
            print(f"{sym:8s} INSUFFICIENT DATA"); sys.stdout.flush(); continue
        summary[sym] = out
        bo = out["best_overall"]; b13 = out.get("best_1to3")
        left = ""
        if bo:
    ...
    md = ["# Per-Pair Optimal Parameters (Fast Sweep)", "",
    ...
    for sym in SYMBOLS:
        ...
        if not bo: continue
        md.append(f"| {sym} | {bo['sl']} | {bo['tp']} | 1:{bo['rr']} | "
                  f"{bo['adx']} | {bo['trades']} | {bo['win_rate']*100:.1f}% | "
                  f"{bo['expectancy']:+.3f} | {bo['gross_r']:+.1f} | "
                  f"{bo['sharpe']:+.2f} |")
    md.append(""); md.append("## Best 1:3 RR (or better) config per symbol")
    md.append("")
    ...
    for sym in SYMBOLS:
        ...
        if not b13:
            md.append(f"| {sym} | — | — | — | — | — | — | — | — |"); continue
        md.append(f"| {sym} | {b13['sl']} | {b13['tp']} | 1:{b13['rr']} | "
