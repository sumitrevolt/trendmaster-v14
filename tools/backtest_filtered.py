...
def backtest_1to3(df: pd.DataFrame, cfg: FilterConfig, name: str = "cfg") -> BacktestResult:
    ...
    while i < n - cfg.hold_bars:
        ...
        if (
            ...
            and (bb_up_v[i] - bb_lo_v[i]) > np.nanmedian(bb_up_v[max(0, i - 50) : i] - bb_lo_v[max(0, i - 50) : i])
...
def grid_search(df: pd.DataFrame) -> List[BacktestResult]:
    ...
    for adx in (25, 30, 35):
        configs.append(
            (f"adx{adx}+peak", FilterConfig(adx_min=adx, require_peak_session=True, sl_atr=1.0, tp_atr=3.0)),
        )
    ...
    for adx in (25, 30):
        configs.append(
            (
                f"adx{adx}+vol45-85",
                FilterConfig(adx_min=adx, vol_min_pct=0.45, vol_max_pct=0.85, sl_atr=1.0, tp_atr=3.0),
            ),
        )
    ...
    for adx in (25, 30, 35):
        configs.append(
            (
                f"adx{adx}+peak+vol",
                FilterConfig(
                    adx_min=adx, require_peak_session=True, vol_min_pct=0.45, vol_max_pct=0.85, sl_atr=1.0, tp_atr=3.0
                ),
            ),
        )
...
def main() -> int:
    ...
    print()
    print(f"{'config':28s} {'trades':>6s} {'WR':>6s} {'exp(R)':>7s} {'grossR':>8s} {'Sharpe':>7s}  {'pass':>5s}")
    print("-" * 80)
    for r in results:
        mark = "PASS" if r.passed else ("near" if r.win_rate >= 0.75 else "")
        print(
            f"{r.name:28s} {r.trades:6d} {r.win_rate * 100:5.1f}% "
    ...
    print()
    if winners:
        print(f"[PASS] {len(winners)} config(s) achieved >= 80% WR at 1:3 RR with >= 30 trades.")
        print("   Top winner:", winners[0].name)
    else:
        best = max(results, key=lambda r: r.win_rate)
        print(
            f"[FAIL] No config achieved 80% WR at 1:3 RR. Best: "
